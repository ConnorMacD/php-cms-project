		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					( substr(strtolower($value),-4) == '.jpg'
							|| substr(strtolower($value),-4) == '.png'
							|| substr(strtolower($value),-5) == '.jpeg'
							|| substr(strtolower($value),-4) == '.gif'
							|| substr(strtolower($value),-5) == '.tiff')
							? true : false;
					...
					if($is_image)
					{
						//Code altered by Amit Shah for generation of thunbnails
						...
						$file_url_anchor .= ' class="image-thumbnail">' .
								$this->img->rimg($_file_url, array('shortside'=>50, 'sharpen'=>true));
						//Code Alteration ends in here
					}
					else
					{
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	public function set_js($js_file, $useBaseUrl=TRUE)
	{
		if($useBaseUrl)
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	/** Changes by Amit Shah  - begins in here**/
	/**
	...
	/**
	 *
	 * Defines the files to be allowed for uploading
	 * @param string types
	 */
	...
	/** Changes by Amit Shah  - ends in here**/
	/**
	 *
